...
exports.starParts = starParts;
exports.nextAward = nextAward;
exports.needsExplanation = needsExplanation;
 ...
 */
function meterParts(value, limit) {
    if (!usable(value)) {
 ...
 */
function nextAward(value, max) {
    const { filled, max: total, hasScale } = starParts(value, max);
    if (!hasScale || filled >= total)
        return undefined;
    return filled + 1;
}
/**
 ...
 */
function needsExplanation(status) {
    switch (status) {
        case 'skipped':
        case 'missed':
        case 'overdue':
...
//# sourceMappingURL=family-v4.js.map