 ...
 */
function VirtualListV4({ className, data, renderItem, keyExtractor, estimatedItemSize, separators = true, emptyText = 'Nothing here yet', loading = false, maxHeight = 480, ...rest }) {
    if (loading) {
        return ((0, jsx_runtime_1.jsx)("div", { className: (0, cn_1.cn)('flex items-center justify-center p-xl', className), ...rest, children: (0, jsx_runtime_1.jsx)("span", { role: "status", "aria-label": "Loading", className: (0, cn_1.cn)('inline-block h-5 w-5 animate-spin rounded-[var(--xen-radius-full)]', 
                // `border-border`, not `border-neutral-300`: the ramps carry the
                // light orientation in both schemes, so the base's track is a
                // bright ring on a dark page.
                'border-2 border-border border-t-primary') }) }));
    }
    if (data.length === 0) {
        return ((0, jsx_runtime_1.jsx)("div", { className: (0, cn_1.cn)(
            // `muted-text`, not `muted`: this line is the only thing on screen
            // when it shows, and `muted` carries no contrast promise.
            'bg-surface p-xl text-center font-body text-sm text-muted-text', className), ...rest, children: emptyText }));
    }
