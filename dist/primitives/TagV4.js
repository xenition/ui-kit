 ...
 */
exports.TagV4 = React.forwardRef(function TagV4({ className, tone = 'neutral', variant = 'solid', size = 'md', removable = false, dot = false, onRemove, children, ...rest }, ref) {
    (0, inject_1.injectStyleOnce)('xen-v4-tag-styles', TAG_V4_CSS);
    const t = TONE[tone];
    ...
    if (variant === 'solid') {
        ...
        fill = (0, cn_1.cn)(`bg-[color-mix(in_srgb,${t.accent}_14%,var(--xen-surface))]`, t.text);
    }
    else {
        fill = (0, cn_1.cn)('bg-surface border', t.ring, t.text);
    }
    return ((0, jsx_runtime_1.jsxs)("span", { ref: ref, "data-xen-v4-tag": "", className: (0, cn_1.cn)('inline-flex items-center gap-xs font-body text-xs font-semibold', 
        // A tag is a word, not a capsule (§8).
        'rounded-[var(--xen-radius-sm)]', SIZE[size], fill, className), ...rest, children: [dot ? ((0, jsx_runtime_1.jsx)("span", { "aria-hidden": true, className: (0, cn_1.cn)('inline-block shrink-0 rounded-full', DOT_SIZE[size], DOT[tone]) })) : null, children, showRemove ? ((0, jsx_runtime_1.jsx)("button", { type: "button", "data-xen-v4-tag-x": "", "aria-label": "Remove", onClick: onRemove, className: "inline-flex shrink-0 items-center justify-center", children: (0, icon_names_1.resolveIconGlyph)('close') })) : null] }));
});
