...
/**
 * A semantic color slot a rarity maps to. Mirrors the subset of the theme's
 * semantic keys the gaming module uses; each resolves to a `--xen-*` token
 * utility class (never a literal color).
 */
export type RaritySlot = 'muted' | 'success' | 'primary' | 'accent' | 'warn';
 ...
 */
export declare function rarityColorKey(rarity?: ItemRarity): RaritySlot;
/** Token `text-*` utility class for a rarity slot (no literal colors). */
export declare const RARITY_TEXT_CLASS: Record<RaritySlot, string>;
/** Token `border-*` utility class for a rarity slot (no literal colors). */
export declare const RARITY_BORDER_CLASS: Record<RaritySlot, string>;
