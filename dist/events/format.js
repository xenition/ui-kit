"use strict";
/**
 * Tiny date/time formatting helpers shared across the web events module. No
 * external deps and no locale libraries — just the fixed short labels the event
 * surfaces need (calendar strips, countdowns). Nothing reads the clock at import
 * time; callers that need "now" pass it in (`CountdownBadge`, `CalendarStrip`).
 * Every array access is guarded by `?? ''` so an out-of-range index never
 * throws.
 */
