 ...
 */
exports.RegisterKeypad = React.forwardRef(function RegisterKeypad({ value = '', onChange, onKeyPress, variant = 'amount', showDisplay = true, displayPrefix, placeholder = '0', maxLength = 12, disabled = false, accessibilityLabel = 'Register keypad', className, ...rest }, ref) {
    const applyKey = (key) => {
        switch (key) {
            case 'backspace':
    ...
    return ((0, jsx_runtime_1.jsxs)("div", { ref: ref, "aria-label": accessibilityLabel, "data-xen-register-keypad": "", className: (0, cn_1.cn)('flex flex-col gap-[var(--xen-space-sm)]', disabled ? 'opacity-50' : '', className), ...rest, children: [showDisplay ? ((0, jsx_runtime_1.jsxs)("div", { "aria-label": `Entry ${value || placeholder}`, className: "flex items-baseline justify-end gap-[var(--xen-space-xs)] rounded-[var(--xen-radius-md)] border border-border bg-surface px-[var(--xen-space-md)] py-[var(--xen-space-sm)]", children: [displayPrefix ? (0, jsx_runtime_1.jsx)("span", { className: "text-lg text-muted", children: displayPrefix }) : null, (0, jsx_runtime_1.jsx)("span", { className: (0, cn_1.cn)('truncate text-2xl font-bold tabular-nums', value ? 'text-on-surface' : 'text-muted'), children: displayText || placeholder })] })) : null, (0, jsx_runtime_1.jsx)("div", { className: "flex flex-col gap-[var(--xen-space-sm)]", children: rows.map((row, r) => ((0, jsx_runtime_1.jsx)("div", { className: "flex gap-[var(--xen-space-sm)]", children: row.map((key) => {
                        const isAction = key === 'backspace' || key === 'clear';
                        return ((0, jsx_runtime_1.jsx)("button", { type: "button", "aria-label": keyLabel(key), disabled: disabled, onClick: () => press(key), className: (0, cn_1.cn)('flex min-h-[52px] flex-1 items-center justify-center rounded-[var(--xen-radius-md)] border border-border bg-surface text-xl font-semibold transition-colors', 'hover:bg-neutral-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-300', 'disabled:pointer-events-none disabled:opacity-50', isAction ? 'text-muted' : 'text-on-surface'), children: keyGlyph(key) }, key));
                    }) }, r))) })] }));
});
