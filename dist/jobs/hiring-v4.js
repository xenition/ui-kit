...
exports.salaryParts = salaryParts;
exports.stageParts = stageParts;
exports.relativeParts = relativeParts;
 ...
 */
function stageParts(stage, stages) {
    const total = stages.length;
    const found = stages.indexOf(stage);
    const known = found >= 0;
    const index = known ? found : 0;
    return { stage: known ? stage : (stages[0] ?? stage), index, total, known };
}
/**
 ...
 */
function relativeParts(iso, now = Date.now()) {
    if (!iso)
        return { value: 0, unit: 'now', valid: false };
    const then = new Date(iso).getTime();
    if (Number.isNaN(then))
        return { value: 0, unit: 'now', valid: false };
    const secs = Math.max(0, Math.floor((now - then) / 1000));
    if (secs < 60)
        return { value: 0, unit: 'now', valid: true };
    const mins = Math.floor(secs / 60);
    if (mins < 60)
        return { value: mins, unit: 'minute', valid: true };
    const hrs = Math.floor(mins / 60);
    if (hrs < 24)
        return { value: hrs, unit: 'hour', valid: true };
    const days = Math.floor(hrs / 24);
    if (days < 30)
        return { value: days, unit: 'day', valid: true };
    const months = Math.floor(days / 30);
    if (months < 12)
        return { value: months, unit: 'month', valid: true };
...
//# sourceMappingURL=hiring-v4.js.map