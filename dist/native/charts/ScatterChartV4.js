...
exports.ScatterChartV4 = ScatterChartV4;
 ...
 *   exact one.
 */
function ScatterChartV4({ data, series, width = 300, height = 200, xDomain, yDomain, showAxes = true, showGrid = true, title, summary, caption, legend, tooltip: _tooltip = true, valueFormat = String, onPointSelect, loading = false, emptyLabel = 'No data', animate: _animate = true, accessibilityLabel, style, }) {
    ...
    const slop = Math.max(((0, nav_v4_1.minTap)(tokens.spacing) - v4_chart_1.CHART_MARK.dotSize) / 2, 0);
    return ((0, jsx_runtime_1.jsxs)(react_native_1.View, { style: [frame, style], children: [header, (0, jsx_runtime_1.jsx)(react_native_1.View, { accessibilityRole: "image", accessibilityLabel: label, children: (0, jsx_runtime_1.jsx)(react_native_svg_1.default, { width: width, height: height, viewBox: `0 0 ${width} ${height}`, children: (0, jsx_runtime_1.jsxs)(react_native_svg_1.G, { children: [showGrid
                                ? Array.from({ length: GRID_LINES }, (_, i) => {
                                    const y = PAD + (plotH * (i + 1)) / (GRID_LINES + 1);
                                    return ((0, jsx_runtime_1.jsx)(react_native_svg_1.Line, { x1: PAD, y1: y, x2: width - PAD, y2: y, stroke: palette.grid, 
                                        // A hairline is the one bare number §4.4 allows.
                                        strokeWidth: 1 }, `grid-${i}`));
                                })
                                : null, showAxes ? ((0, jsx_runtime_1.jsxs)(jsx_runtime_1.Fragment, { children: [(0, jsx_runtime_1.jsx)(react_native_svg_1.Line, { x1: PAD, y1: height - PAD, x2: width - PAD, y2: height - PAD, stroke: palette.axis, strokeWidth: v4_chart_1.CHART_MARK.stroke }), (0, jsx_runtime_1.jsx)(react_native_svg_1.Line, { x1: PAD, y1: PAD, x2: PAD, y2: height - PAD, stroke: palette.axis, strokeWidth: v4_chart_1.CHART_MARK.stroke })] })) : null, resolved.map((s, si) => s.points.map((p, pi) => ((0, jsx_runtime_1.jsx)(react_native_svg_1.Circle, { testID: "scatter-point", cx: px(p.x), cy: py(p.y), r: v4_chart_1.CHART_MARK.dotSize / 2, fill: s.tone !== undefined ? colors[s.tone] : (0, internal_v4_1.chartSlotColor)(palette, si), stroke: palette.ring, strokeWidth: v4_chart_1.CHART_MARK.ring, ...(onPointSelect !== undefined
                                    ? {
                                        onPress: () => onPointSelect(p, si, pi),
                                        hitSlop: { top: slop, bottom: slop, left: slop, right: slop },
                                        accessibilityRole: 'button',
                                        accessibilityLabel: `${s.label}${p.label !== undefined ? ` · ${p.label}` : ''}: ${valueFormat(p.x)}, ${valueFormat(p.y)}`,
                                    }
                                    : {}) }, `${s.key}-${pi}`))))] }) }) }), showLegend ? (0, jsx_runtime_1.jsx)(LegendV4_1.LegendV4, { items: legendItems }) : null, footer] }));
}
