...
import { type AuthBrandTileShape, type AuthBrandTileSize } from './AuthBrandTileV4';
import type { CardPadding, CardVariant } from './Card';
import type { AuthCardProps } from './AuthCard';
/**
 * `AuthCard`, V4 — the shell every auth screen is assembled in. Native twin of
 * the web `AuthCardV4`, at prop parity.
 *
 ...
 *
 * The base sets `gap: spacing.md` on the card and stops, so the distance from
 * the brand tile to the headline, from the headline to the first field, and
 * from the last field to the footer are all the same 16 — and the card reads as
 * five things stacked at equal pitch rather than as four bands. §4 is explicit:
 ...
 *
 * ## 2. The column was a literal, not the seed's scale
 *
 * The base writes `maxWidth: 384` here and `max-w-sm` on web. They agree today
 * by coincidence — Tailwind's `sm` is 24rem — and neither follows the theme: a
 * seed that tightens its spacing scale gets tighter padding, tighter gaps and a
 * card that stays exactly 384 wide, so the one box holding everything is the
 * one box out of rhythm with its contents. {@link AuthCardV4Props.width}
 * composes the column from the spacing scale instead: `2xl × 8` is **384 at the
 * default scale** — the same pixels, now derived. It is the argument
 * `AuthHeadingV4` makes for its measure (`2xl × 10`) and `AuthBrandTileV4`
 * makes for its square, and `'md'` deliberately lands on that same 480 so a
 * wide auth card and a headline measure agree instead of nearly agreeing.
 *
 ...
 * this twin wraps a **string** in `Text size="sm" tone="muted"`. So the same
 * `footer="Don't have an account?"` is body-sized ink on web and a small muted
 * line on native — a parity break hiding inside a component whose whole job is
 * to look the same on both. V4 states it once, in both twins: a string footer
 * is `sm`/`mutedText`, centred; any other node is rendered as given, so §9's
 * "Don't have an account? **Register**" (which needs a pressable inside it)
 * still works.
 *
 ...
 * alternative is every caller hand-rolling a border.
 *
 * ## What it deliberately does not have
 *
 * **No motion.** `internal/motion-v4.ts` exists so a component with a state
 ...
 * one. A shell that forced `3xl` could not be embedded in a sheet.
 *
 ...
 *
 * Token-bound throughout; the only numbers are the spacing multiples above.
 */
...
export interface AuthCardV4Props extends AuthCardProps {
     ...
     * scale and so the base's `maxWidth: 384` to the pixel. `'md'` is `2xl × 10`
     ...
     *
     * The base gave the shell no way to reach the tile's `accessibilityLabel`, so
     * an app whose mark carries meaning ("Acme") had to drop out of `AuthCard`
     * entirely to say so.
     */
    brandLabel?: string;
    /**
...
export declare function AuthCardV4({ title, subtitle, children, footer, brandGlyph, brandIcon, align, titleSize, variant, padding, width, brandSize, brandShape, brandLabel, footerDivider, style, }: AuthCardV4Props): React.ReactElement;
//# sourceMappingURL=AuthCardV4.d.ts.map