...
/**
 * The hairline's thickness. `1` is the one bare number §10.1 names outright,
 * and it is the same `1` the web twin gets from `h-px`, so the two platforms
 * draw the same weight of line.
 */
const HAIRLINE = 1;
/**
 * **V4 auth divider** — the "or continue with" separator of
 * `ONBOARDING-DESIGN-SPEC.md` §9, in the V4 design line. Native twin of the web
 * `AuthDividerV4`, at prop parity.
 *
 ...
 * which point the patch becomes a visible pale smear across the hairline. Here
 * the rule is two `flex: 1` segments with the label between them, so it is
 ...
 *
 * The hairline stays a hairline: `1` unit of `colors.border`, never a thicker
 * rule or a `neutral` step used as a line. The label steps up from the base's
 * `xs` to `sm` — this line is a consumer mobile surface, and `xs` on a phone is
 * below the size at which incidental copy is comfortably read.
 */
function AuthDividerV4({ label, align = 'center', children, style, }) {
    ...
    if (slotted && isEmptyNode(children))
        return null;
    const rule = ((0, jsx_runtime_1.jsx)(react_native_1.View, { style: { flex: 1, height: HAIRLINE, backgroundColor: colors.border } }));
    return ((0, jsx_runtime_1.jsxs)(react_native_1.View, { style: [{ width: '100%', gap: tokens.spacing.md }, style], children: [(0, jsx_runtime_1.jsx)(react_native_1.View, { style: {
                    flexDirection: 'row',
                    alignItems: 'center',
                    gap: tokens.spacing.md,
                }, children: label ? ((0, jsx_runtime_1.jsxs)(jsx_runtime_1.Fragment, { children: [align === 'start' ? null : rule, (0, jsx_runtime_1.jsx)(TextV4_1.TextV4, { size: "sm", tone: "muted", children: label }), align === 'end' ? null : rule] })) : (
                ...
                rule) }), slotted ? (0, jsx_runtime_1.jsx)(react_native_1.View, { style: { gap: tokens.spacing.sm }, children: children }) : null] }));
}
