 ...
 * Native twin of the web `AuthBrandTileV4`, at prop parity.
 *
 ...
 *
 * V4 composes the square from `tokens.spacing` instead: `2xl + sm`, which is
 * **56 at the default scale** — the same pixels, the same §9 tile, now derived
 * rather than typed. It is the argument `internal/identity-v4.ts` already
 * makes for avatar diameters, and it has the pleasant side effect that a `md`
 * tile and an `AvatarV4` at `lg` are the same square, so a header carrying
 * both lines up.
 *
 ...
 *
 * The disc derives its radius from **half the box**, not from `radius.full`.
 * `radius.full` compiles to `0` on a `sharp` seed — the Addendum records the
 * same trap for `Switch`, which derives its track radius from its own height
 * for exactly this reason. A ratio of a box the component already owns is
 * geometry, which §10.1 allows; a radius token that silently squares off the
 * one round thing on the screen is a bug.
 *
 * ## A note on the fill
 *
 ...
 *
 * **No motion.** `internal/motion-v4.ts` and its web sibling exist so that a
 * component with a state change takes M3's duration rather than inventing one.
 * This component has no states: it does not press, focus or disable. An
 * entrance animation would be decoration invented for its own sake, so there
 * is none, and there is no duration in this file to drift.
 *
 * **No gradient.** §35.11 keeps the brand sweep for the hero and the single
 * primary action, and `ButtonV4` already spends it on the CTA.
 *
...
export declare function AuthBrandTileV4({ glyph, name, align, size, shape, accessibilityLabel, style, }: AuthBrandTileV4Props): React.ReactElement | null;
//# sourceMappingURL=AuthBrandTileV4.d.ts.map