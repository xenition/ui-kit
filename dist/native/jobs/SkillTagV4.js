 ...
 */
function SkillTagV4({ label, variant = 'default', selected = false, onPress, onRemove, removeLabel = 'Remove', variantLabels, style, }) {
    const theme = (0, theme_1.useXenitionTheme)();
    const { colors, tokens } = theme;
    if (!label)
        return null;
    const tone = VARIANT_TONE[variant];
    const ground = tone ? (0, tone_v4_1.toneFill)(theme, tone) : colors.card;
    const ink = tone ? (0, tone_v4_1.onPair)(theme, tone) : colors.onCard;
    ...
    const text = ((0, jsx_runtime_1.jsx)(TextV4_1.TextV4, { size: "xs", weight: "medium", numberOfLines: 1, style: { color: ink }, children: `${MARKER[variant]}${label}` }));
    const body = onPress ? ((0, jsx_runtime_1.jsx)(react_native_1.Pressable, { accessibilityRole: "button", accessibilityLabel: (0, tone_v4_1.spokenName)([label, meaning]), accessibilityState: { selected }, onPress: onPress, style: ({ pressed }) => [
            bodyPad,
            { backgroundColor: pressed ? (0, state_v4_1.pressOver)(theme, ground, ink) : 'transparent' },
        ], children: text })) : ((0, jsx_runtime_1.jsx)(react_native_1.View, { accessible: true, accessibilityLabel: (0, tone_v4_1.spokenName)([label, meaning]), style: bodyPad, children: text }));
    return ((0, jsx_runtime_1.jsxs)(react_native_1.View, { style: [chip, style], children: [body, onRemove ? ((0, jsx_runtime_1.jsx)(react_native_1.Pressable, { accessibilityRole: "button", accessibilityLabel: (0, tone_v4_1.spokenName)([removeLabel, label]), onPress: onRemove, style: ({ pressed }) => ({
                    alignItems: 'center',
                    justifyContent: 'center',
                    minWidth: tap,
                    minHeight: tap,
                    backgroundColor: pressed ? (0, state_v4_1.pressOver)(theme, ground, ink) : 'transparent',
                }), children: (0, jsx_runtime_1.jsx)(TextV4_1.TextV4, { size: "xs", weight: "semibold", style: { color: ink }, children: "\u00D7" }) })) : null] }));
}
