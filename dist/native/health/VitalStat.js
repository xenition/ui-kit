 ...
 * `onPress` is provided.
 */
function VitalStat({ variant, value, unit, label, delta, onPress, appearance = 'classic', style, }) {
    const { colors, tokens } = (0, theme_1.useXenitionTheme)();
    const meta = VARIANT_META[variant];
    const resolvedUnit = unit ?? meta.unit;
    const resolvedLabel = label ?? meta.label;
    const deltaColor = delta == null || delta === 0 ? colors.muted : delta > 0 ? colors.successText : colors.dangerText;
