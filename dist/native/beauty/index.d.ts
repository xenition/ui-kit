/**
 * `@xenition/ui/native/beauty` — composed salon / spa / beauty-services blocks
 * for React Native. Mobile-first and presentational only: every component takes
 * shaped data plus callbacks (nothing fetches, no SDK import) and is styled
 * exclusively from the compiled theme via `useXenitionTheme()`, so a seed change
 * (dark mode included) restyles the whole set. No literal colors — only
 * `SemanticColors` slots plus `withAlpha` tints — and no external dependencies:
 * `BeforeAfter` is a styled split/toggle built from plain `View`s + `Image`,
 * no gesture library. Money is always integer **cents**, formatted through the
