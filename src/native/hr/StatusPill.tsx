...
import { toneColor, type StatusMeta } from './internal';
...
export function StatusPill({
  ...
  const fg = solid
    ? // On a solid fill, prefer the paired on-token when the tone is a true slot.
      meta.tone === 'success'
      ? colors.onSuccess
      : meta.tone === 'danger'
        ? colors.onDanger
  ...
  return (
    ...
    >
      <Text
        ...
        style={{ color: fg, fontSize: tokens.typography.scale[textKey], lineHeight: tokens.typography.scale[textKey] * 1.2 }}
      >
        {meta.glyph}
      </Text>
      <Text
        style={{ color: fg, fontSize: tokens.typography.scale[textKey], fontWeight: '600' }}
      >
        {meta.label}
      </Text>
    </View>
