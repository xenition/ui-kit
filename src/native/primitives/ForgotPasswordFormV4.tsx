...
export function ForgotPasswordFormV4({
  ...
  return (
    <AuthCardV4
      ...
      footer={
        onLoginClick ? (
          <AuthSwitchFooterV4 prompt={backPrompt} label={backLabel} onPress={onLoginClick} />
        ) : undefined
      }
    >
      {sent ? (
        ...
        >
          ...
          {sentTo ? (
            <TextV4 size="sm" weight="semibold" align="center" testID="xen-v4-forgot-address">
              {sentTo}
            </TextV4>
          ) : null}
          {resendable ? (
        ...
        <FormV4 testID="xen-v4-forgot-form">
          {submitError ? <AlertV4 tone="danger">{submitError}</AlertV4> : null}
          <AuthFieldV4
            label={emailLabel}
            icon="mail"
