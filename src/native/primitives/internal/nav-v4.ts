...
import { EASING_ENTER, V4_MOTION } from './motion-v4';
 ...
 */
export function panelMinWidth(spacing: SpacingScale): number {
 ...
 */
export function useMovingIndicator(activeIndex: number): MovingIndicator {
  const reduced = useReducedMotion();
  const slots = React.useRef<Record<number, { x: number; w: number }>>({});
  ...
  const onItemLayout = React.useCallback(
    (index: number) => (event: LayoutChangeEvent) => {
      ...
      if (previous !== undefined && previous.x === x && previous.w === w) return;
