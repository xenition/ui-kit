...
export function RatingV4({
  ...
  const row = (color: string): React.ReactElement => (
    <View style={{ flexDirection: 'row' }}>
      {Array.from({ length: total }, (_, i) => (
        <Text key={i} style={{ color, fontSize, letterSpacing: 1, flexShrink: 0 }}>
          {STAR}
        </Text>
      ))}
    </View>
  ...
  return (
    ...
    >
      <View style={{ position: 'relative' }}>
        {row(colors.mutedText)}
        <View
          testID="xen-v4-rating-fill"
          pointerEvents="none"
          style={{
            position: 'absolute',
            left: 0,
        ...
        >
          {/* `accentText`, not `accent` — a filled star IS the text here. */}
          {row(colors.accentText)}
        </View>
      </View>
      {showValue ? (
        <Text
