 ...
 */
export function verdictTone(verdict: RangeVerdict | undefined): ToneV4 {
  switch (verdict) {
    case 'in-range':
      return 'success';
    case 'low':
...
export const RANGE_LABEL: Record<RangeVerdict, string> = {
  low: 'Below range',
  'in-range': 'In range',
  high: 'Above range',
};
 ...
 */
export function deltaTone(delta: number | undefined, lowerIsBetter = false): ToneV4 {
 ...
 */
export function looseCardStyle(
  theme: XenitionNativeTheme,
  appearance: Appearance | undefined
): ViewStyle | null {
  if (appearance === undefined || appearance === 'classic') return null;
  const { colors, tokens } = theme;
  return {
    ...appearanceStyle(appearance, colors, tokens),
    borderRadius: tokens.radius.lg,
