/**
 * Shared internals for the native government / civic module. Money re-uses the
 * single kit-wide `formatMoney` home (integer **cents** → two-decimal localized
 * string, no float drift), and `withAlpha` derives a translucent tint from a
 * resolved token hex so tints never introduce a literal color (mirrors the
 * primitives' pattern).
 */
