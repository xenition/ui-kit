...
export function BidRowV4({
  ...
  showAvatar = true,
  style,
}: BidRowV4Props): React.ReactElement | null {
  ...
  const leadingNode =
    typeof rank === 'number' ? (
      <TextV4
        size="sm"
        weight="semibold"
        tone={leading ? 'onSelected' : 'mutedText'}
        numeric="tabular"
      >
        {rank}
      </TextV4>
  ...
  return (
    <View
      accessibilityLabel={`${leading ? 'Leading bid, ' : ''}${name}, ${amount}`}
      style={[
        rowContainerStyle(theme, { twoLine: supporting }),
        { backgroundColor: rowGround(theme, { selected: leading }) },
        style,
      ]}
    >
      ...
      <View style={rowTextStyle(theme)}>
        ...
        >
          <TextV4 size="base" weight="semibold" tone={ink} numberOfLines={1} style={{ flexShrink: 1 }}>
            {name}
          </TextV4>
          {leading ? (
