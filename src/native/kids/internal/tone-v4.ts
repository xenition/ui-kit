 ...
 */
export const MOOD_GLYPH: Record<ChildMood, string> = {
  happy: '😊',
  excited: '🤩',
  calm: '😌',
  sad: '😢',
...
export const MOOD_LABEL: Record<ChildMood, string> = {
  happy: 'Happy',
  excited: 'Excited',
  calm: 'Calm',
  sad: 'Sad',
 ...
 * radius to drift.
 */
export function cardStyle(theme: XenitionNativeTheme): ViewStyle {
 ...
 * clipping.
 */
export function rowShellStyle(theme: XenitionNativeTheme): ViewStyle {
  const { colors, tokens } = theme;
  return {
    flexDirection: 'row',
    alignItems: 'center',
...
export function skeletonBlockStyle(
  ...
  options: { height: number; width?: ViewStyle['width']; round?: boolean }
): ViewStyle {
  const { tokens } = theme;
  return {
 ...
 */
export function trackGround(theme: XenitionNativeTheme): string {
 ...
 */
export function tapTargetStyle(theme: XenitionNativeTheme): ViewStyle {
  const size = minTap(theme.tokens.spacing);
  return {
    minWidth: size,
    minHeight: size,
    alignItems: 'center',
