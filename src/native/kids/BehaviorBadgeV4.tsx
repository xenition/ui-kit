...
import { IDENTITY_TONE, spokenLine, tapTargetStyle } from './internal/tone-v4';
...
const TONE_MARK: Record<BehaviorTone, { glyph: string; sign: string }> = {
  positive: { glyph: '👍', sign: '+' },
  negative: { glyph: '👎', sign: '−' },
  neutral: { glyph: '•', sign: '' },
};
 ...
 */
export function BehaviorBadgeV4({
  label,
  tone = 'neutral',
  points,
  icon,
  ...
  onPress,
  style,
}: BehaviorBadgeV4Props): React.ReactElement | null {
  ...
  const name = spokenLine([label, word, pointsText, note]);

  const chip = (
    <BadgeV4 tone={IDENTITY_TONE} variant="soft" size={size}>
      {[glyph, label, pointsText ? `(${pointsText})` : null].filter(Boolean).join(' ')}
    </BadgeV4>
  );
  ...
  if (!onPress) {
    return (
      <View accessible accessibilityLabel={name} style={[{ alignSelf: 'flex-start' }, style]}>
        {body}
      </View>
    );
  }
