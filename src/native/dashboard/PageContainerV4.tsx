...
export interface PageContainerV4Props
  extends Omit<PageContainerProps, 'children' | 'style'> {
  /**
 ...
 */
export function PageContainerV4({
  title,
  subtitle,
  headerAction,
  scroll = true,
  ...
  style,
  testID,
}: PageContainerV4Props): React.ReactElement {
  ...
  const ground: ViewStyle = { flex: 1, backgroundColor: colors.surface };

  if (!scroll) {
    return (
      <View testID={testID} style={[ground, pad, style]}>
        {header}
        {children}
      </View>
    );
