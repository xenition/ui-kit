...
export function MediaFigureV4({
  ...
  videoLabel = 'video',
  style,
}: MediaFigureV4Props): React.ReactElement {
  const theme = useXenitionTheme();
  const { tokens } = theme;

  const ratio = reserveAspect && item.width && item.height ? item.width / item.height : undefined;
  const name = [item.alt ?? item.caption ?? openLabel, isVideo(item) ? videoLabel : null]
    .filter(Boolean)
    .join(', ');

  const surface = <MediaSurfaceV4 item={item} aspectRatio={ratio} inButton={Boolean(onActivate)} />;
