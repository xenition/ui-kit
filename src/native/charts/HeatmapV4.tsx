...
import { CHART_MARK, CHART_RAMP_STEPS } from '../../primitives/internal/v4-chart';
import { ChartEmptyV4, useChartPaletteV4 } from './internal-v4';
 ...
 */
const RAMP_STEPS = CHART_RAMP_STEPS;
...
const bucket = (t: number): number => Math.round(clamp01(t) * (RAMP_STEPS - 1)) / (RAMP_STEPS - 1);
...
export function HeatmapV4({
  ...
  accessibilityLabel,
  style,
}: HeatmapV4Props): React.ReactElement {
  ...
  if (rows === 0 || cols === 0) {
    return (
      <View style={[frame, style]}>
        {header}
        {/*
          §4.5: never a bare string and never `null`, and the footprint is kept
          so the screen does not reflow when the data lands. The shared
          `ChartEmptyV4` now exists in `internal-v4.tsx`; this used to be a
          local copy of it, written while that module was closed to the build
          groups. The footprint is the grid's own — at least one cell tall, so
          a one-row heatmap does not reserve less than it will draw.
        */}
  ...
  return (
    <View style={[frame, style]}>
      ...
      <View style={{ flexDirection: 'row', gap: tokens.spacing.sm }}>
        {rowLabels !== undefined ? (
          <View style={{ gap: CHART_MARK.gap }}>
            {data.map((_, r) => (
              <View key={r} style={{ height: edge, justifyContent: 'center' }}>
        ...
        <View style={{ gap: tokens.spacing.xs }}>
          ...
          >
            {data.map((row, r) => (
              <View key={r} style={{ flexDirection: 'row', gap: CHART_MARK.gap }}>
                {Array.from({ length: cols }, (_, c) => {
                  ...
                  if (!interactive) return <View key={c} testID="heatmap-cell" style={tile} />;
                  return (
                    <Pressable
                      key={c}
                      testID="heatmap-cell"
