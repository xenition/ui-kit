...
import { DECORATIVE, POLICY_LINE_V4, chipGround, spokenLine } from './internal/tone-v4';
...
const DEFAULT_LABELS: Required<InsuranceIdCardV4Labels> = {
 ...
 */
export function InsuranceIdCardV4({
  carrier,
  policyNumber,
  insured,
  subject,
  ...
  testID,
  style,
}: InsuranceIdCardV4Props): React.ReactElement | null {
  ...
  const field = (caption: string, value: string | undefined): React.ReactElement | null =>
    value == null || value === '' ? null : (
      <View
        accessible
        accessibilityLabel={spokenLine([caption, value])}
        key={caption}
        style={{ flexGrow: 1, flexBasis: '45%', gap: tokens.spacing.xs / 2 }}
      >
  ...
  return (
    ...
    >
      <View
        accessible
        accessibilityLabel={spokenLine([carrier, line.label, term])}
      ...
      <View
        accessible
        accessibilityLabel={spokenLine([copy.policyNumber, policyNumber])}
        style={{
      ...
      >
        ...
        {/* Legible at arm's length, and grouped digits that stay aligned. */}
      ...
      >
        {field(copy.insured, insured)}
        {field(copy.subject, subject)}
        {field(copy.effective, effectiveDate)}
        {field(copy.expires, expiryDate)}
        {field(copy.issuerCode, issuerCode)}
      </View>
    </CardV4>
  );
}
