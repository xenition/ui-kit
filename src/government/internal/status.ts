 ...
 * reinforcement, not the sole signal. `tone` values map to `BadgeTone` / the
 * semantic token slots (approved/issued → success, denied → danger). Mirror of
 * the native module's `internal/status.ts` (the native `accent` tone has no web
 * `BadgeTone`, so component-local maps fold it into `primary`).
 */
