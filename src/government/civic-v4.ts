...
export function statusSentence(
  ...
  format?: (label: string, step: number, total: number) => string
): string {
  if (typeof step !== 'number' || typeof total !== 'number' || total <= 0) return label;
  const position = Math.min(Math.max(1, Math.round(step) + 1), Math.round(total));
  return (format ?? ((l, s, t) => `${l}, step ${s} of ${t}`))(label, position, Math.round(total));
}
 ...
 */
export function labelledId(label: string, value: string | undefined): string | undefined {
