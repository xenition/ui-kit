/**
 * `@xenition/ui/utilities` — presentational utility / energy / bill-pay blocks
 * for React DOM. Composed from the web primitives (`Card`, `Button`, `Icon`,
 * `Badge`, `Progress`, `Switch`) and the token-bound `charts` (`BarChart` /
 * `LineChart` / `ProgressRing`), styled exclusively from the `--xen-*` token
 * classes — no literal colors (status tints are opacity-modifier / semantic
 * token classes, never a hex). Money is always carried as integer **cents** and
 * funnelled through the single `formatMoney` home, so printed values never drift,
 * and metered quantities run through `formatUsage`. Bill / service / payment /
 * request / outage status is conveyed by **text + glyph + color** (paid →
 * success, overdue/outage → danger) — never color alone. Every component takes
 * data + callbacks + variants/states (no fetching, no SDK import). Web parity of
