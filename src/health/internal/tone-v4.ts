 ...
 * what surface it is.
 */
export const HEALTH_CARD_CLASS = 'rounded-[var(--xen-radius-lg)] p-lg';

/** The same shell one step down, for a tile rather than a card. */
export const HEALTH_TILE_CLASS = 'rounded-[var(--xen-radius-md)] p-md';
 ...
 */
export function appearanceClass(appearance: Appearance = 'classic'): string {
  switch (appearance) {
    case 'elevated':
      return 'border-0 bg-card text-on-card shadow-[var(--xen-elevation-card)]';
    case 'soft':
      return [
        'border border-[color-mix(in_srgb,var(--xen-primary)_14%,transparent)]',
        'bg-[color-mix(in_srgb,var(--xen-primary)_6%,var(--xen-card))] text-on-card',
      ].join(' ');
    case 'outline':
    ...
    case 'filled':
      // The recessed-tray step the V4 data tables band with, mixed from the two
      // scheme-resolved slots so it darkens a light page and lightens a dark one
      // — where native's `ramps.neutral[100]` is a near-white in both schemes.
      return 'border-0 bg-[color-mix(in_srgb,var(--xen-on-surface)_4%,var(--xen-surface))] text-on-surface';
    case 'classic':
    default:
      return 'border border-border bg-card text-on-card';
  }
}
 ...
 */
export function frameClass(appearance: Appearance = 'classic'): string | undefined {
  return appearance === 'classic'
    ? undefined
    : `${HEALTH_CARD_CLASS} ${appearanceClass(appearance)}`;
}
 ...
 */
export function appearanceStateVars(appearance: Appearance = 'classic'): React.CSSProperties {
  const bare = appearance === 'outline' || appearance === 'minimal';
  return stateGroundVars(
    bare ? 'var(--xen-surface)' : 'var(--xen-card)',
    bare ? 'var(--xen-on-surface)' : 'var(--xen-on-card)'
 ...
 */
export const TRACK_CLASS = 'bg-[color-mix(in_srgb,var(--xen-on-card)_12%,var(--xen-card))]';

/** {@link TRACK_CLASS} as a raw colour, for an SVG `stroke` or `fill`. */
export const TRACK_VAR = 'color-mix(in srgb, var(--xen-on-card) 12%, var(--xen-card))';
 ...
 */
export const VERDICT_TONE: Record<RangeVerdict, ToneV4> = {
  low: 'warn',
  'in-range': 'success',
  high: 'warn',
};
...
export const VERDICT_LABEL: Record<RangeVerdict, string> = {
  low: 'Low',
  'in-range': 'In range',
  high: 'High',
};
 ...
 */
export function spokenLine(parts: ReadonlyArray<string | number | false | null | undefined>): string {
  return parts
    .filter((part): part is string | number => part != null && part !== false && part !== '')
    .map(String)
    .join(', ');
}
