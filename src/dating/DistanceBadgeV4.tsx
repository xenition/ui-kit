 ...
 *    a card.
 */
export const DistanceBadgeV4 = React.forwardRef<HTMLSpanElement, DistanceBadgeV4Props>(
  function DistanceBadgeV4(
    {
      distance,
      unit = 'km',
      nearbyThreshold = 1,
    ...
    const rounded =
      distance == null ? 0 : distance < 10 ? Math.round(distance * 10) / 10 : Math.round(distance);
    const text = isNearby ? nearbyLabel : `${rounded} ${unit} away`;

    return (
      <BadgeV4 ref={ref} tone={tone} variant={variant} aria-label={text} {...rest}>
        {glyph ? (
