 ...
 */
export const PHOTO_SCRIM = 'rgba(0, 0, 0, 0.55)';
export const PHOTO_SCRIM_STRONG = 'rgba(0, 0, 0, 0.72)';
 ...
 */
export function deckPosition(index: number, total: number, format?: (i: number, n: number) => string): string {
  const safeTotal = Math.max(0, Math.floor(total));
  const safeIndex = Math.min(Math.max(0, Math.floor(index)), Math.max(0, safeTotal - 1));
  return (format ?? ((i: number, n: number) => `Profile ${i + 1} of ${n}`))(safeIndex, safeTotal);
}
 ...
 * reversible.
 */
export function canRewind(history: readonly unknown[]): boolean {
