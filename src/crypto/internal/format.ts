...
/** Semantic tone slot for a signed change — gains, losses, or flat. */
export type ChangeTone = 'success' | 'danger' | 'muted';
...
export function truncateHash(hash: string, lead = 6, tail = 4): string {
  ...
  if (h.length <= safeLead + safeTail + 1) return h;
 ...
 */
export function changeToneKey(delta: number): ChangeTone {
...
export function changeGlyph(delta: number): string {
  const safe = Number.isFinite(delta) ? delta : 0;
  if (safe > 0) return '▲';
  if (safe < 0) return '▼';
  return '•';
...
export function changeToneClass(tone: ChangeTone): string {
