 ...
 */
export const STATUS_ANATOMY: Record<StatusKey, StatusAnatomy> = {
 ...
 */
export const STATUS_PREFIX = 'Order status: ';

/** The full announcement for a status badge showing `label`. */
export function statusAnnouncement(label: string): string {
  return `${STATUS_PREFIX}${label}`;
}
