 ...
 */
export const COVER_PAPER_STEPS: readonly number[] = [500, 600, 700];
 ...
 * other and dissolve.
 */
export const COVER_INK_STEPS: readonly number[] = [200, 300, 400];
 ...
 */
export function resolveCoverPlate(
  seed: string | number,
  form?: CoverForm,
  ink?: CoverColorRole,
  paper?: CoverColorRole
  ...
  const paperStep = COVER_PAPER_STEPS[(hash >>> 5) % COVER_PAPER_STEPS.length] as number;
  const inkStep = COVER_INK_STEPS[(hash >>> 13) % COVER_INK_STEPS.length] as number;

  return {
    form: form ?? (COVER_FORMS[hash % COVER_FORMS.length] as CoverForm),
