 ...
 */
const TREND_TONE: Record<StatCardV4Trend, TextTone> = {
  up: 'successText',
  down: 'dangerText',
  flat: 'mutedText',
};
...
export const STAT_CARD_V4_STYLE_ID = 'xen-v4-stat-card-styles';
 ...
 */
export const StatCardV4 = React.forwardRef<HTMLDivElement, StatCardV4Props>(function StatCardV4(
  {
    label,
    value,
    delta,
  ...
  injectStyleOnce(STAT_CARD_V4_STYLE_ID, STAT_CARD_V4_CSS);
  ...
  return (
    <CardV4
      ref={ref}
      data-xen-v4-stat-card=""
      variant={raised ? 'elevated' : 'outlined'}
      radius="lg"
      padding="lg"
      aria-label={`${String(label ?? '')}${valueText ? `: ${valueText}` : ''}${
        delta ? `, ${delta}` : ''
      }`}
      ...
      */}
      <div className="flex min-w-0 flex-col gap-xs">
        ...
        {delta ? (
          ...
          >
            <IconV4 name={TREND_ICON[resolvedTrend]} size="xs" />
            {delta}
          </TextV4>
        ) : null}
        {caption ? (
