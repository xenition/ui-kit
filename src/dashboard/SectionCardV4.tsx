 ...
 */
export const DASHBOARD_CARD_V4_STYLE_ID = 'xen-v4-dashboard-card-styles';
 ...
 */
export const DASHBOARD_CARD_V4_GROUND_ATTR = 'data-xen-v4-card-ground';
 ...
 * that mistake.
 */
export const DASHBOARD_CARD_V4_CSS = `
[data-xen-v4-card][${DASHBOARD_CARD_V4_GROUND_ATTR}="card"] {
 ...
 */
const CARD_PAD_VALUE: Record<CardPadding, string> = {
  none: '0px',
  sm: 'var(--xen-space-sm)',
  md: 'var(--xen-space-md)',
  lg: 'var(--xen-space-lg)',
 ...
 */
export const SectionCardV4 = React.forwardRef<HTMLDivElement, SectionCardV4Props>(
  function SectionCardV4(
    {
      title,
      subtitle,
      action,
    ...
    return (
      <CardV4
        ...
        padding="none"
        data-xen-v4-section-card=""
        // Written out rather than spread from a computed key, because a JSX
        // attribute name is not an expression. `DASHBOARD_CARD_V4_GROUND_ATTR`
        // is the same string and both specs assert that they still agree.
        ...
        className={cn('flex flex-col overflow-hidden', SLOT_GAP, className)}
        style={{ [CARD_PAD_VAR]: CARD_PAD_VALUE[padding], ...style } as React.CSSProperties}
        {...rest}
      >
        {hasHeader ? (
          <div
            data-xen-v4-section-card-header=""
            className={cn('flex flex-row items-start justify-between', SLOT_GAP, PAD_X, PAD_T)}
          >
            {hasText ? (
              ...
              <div className={cn('flex min-w-0 flex-1 flex-col', TEXT_GAP)}>
                {title ? (
            ...
            {action ? (
              ...
              <div data-xen-v4-section-card-action="" className="shrink-0">
                {action}
              </div>
            ) : null}
          </div>
        ...
        {/*
          The header rule. Only ever *between* two things: a rule under a header
          with no body below it is a line drawn against the card floor, which is
          the admin styling §4.4 takes off `PageHeader`. Flush, because a header
          is not a row and has no leading slot to clear.
        */}
        {divided && hasHeader && hasBody ? (
          <ListSeparatorV4 data-xen-v4-section-card-rule="" />
        ) : null}

        <div
          data-xen-v4-section-card-body=""
          className={grouped ? undefined : cn(PAD_X, PAD_B, hasHeader ? undefined : PAD_T)}
        >
          {rows.length > 0 ? (
            grouped ? (
              rows.map((row, i) => (
                <React.Fragment key={i}>
                  {/*
                    Between the rows, so the last row gets none — the list ends
                    on the card's own edge, not on a rule hanging in space
                    (§4.4).
                  */}
                  {i > 0 ? (
