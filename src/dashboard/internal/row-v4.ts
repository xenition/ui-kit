...
import { MIN_TAP } from '../../primitives/internal/nav-v4';
...
export const ROW_V4_STYLE_ID = 'xen-v4-dashboard-row-styles';
...
export const ROW_V4_METRICS = {
   ...
   */
  separatorInset: `calc(${MIN_TAP} + var(--xen-space-md))`,
} as const;
 ...
 */
export const ROW_V4_BASE_CLASS = [
 ...
 */
export function rowHeightClass(twoLine: boolean): string {
  return twoLine
 ...
 */
export const ROW_V4_LEADING_CLASS = [
 ...
 */
export const ROW_V4_TEXT_CLASS = 'flex min-w-0 flex-1 flex-col gap-xs text-left';
 ...
 */
export const ROW_V4_TRAILING_CLASS = 'flex shrink-0 items-center gap-sm';
 ...
 */
export function rowGroundClass(selected = false): string {
 ...
 */
export function rowStateVars(
  ground: string = 'var(--xen-card)',
  ink: string = 'var(--xen-on-card)'
): React.CSSProperties {
  return stateGroundVars(ground, ink) as React.CSSProperties;
}
 ...
 */
export function rowSeparatorClass(inset = false): string {
  return inset
