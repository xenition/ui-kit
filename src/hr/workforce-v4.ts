 ...
 */
export function ratingParts(rating: number, max?: number): RatingParts {
 ...
 */
export function hoursParts(hours: number, overtimeHours?: number): HoursParts {
 ...
 */
export function deductionParts(cents: number): AmountParts {
  const c = Math.round(finite(cents));
  if (c === 0) return { direction: 'zero', magnitudeCents: 0 };
