...
import { useChartV4 } from './internal-v4';
import { CHART_SERIES_COUNT } from '../primitives/internal/v4-chart';
...
export const MINI_BAR_V4_STYLE_ID = 'xen-v4-mini-bar-styles';

const slotRule = (i: number): string =>
  `[data-xen-v4-minibar-fill][data-slot="${i}"] { background-color: var(--xen-chart-${i + 1}); }`;
const toneRule = (tone: ChartToneV4): string =>
  `[data-xen-v4-minibar-fill][data-tone="${tone}"] { background-color: var(--xen-${tone}); }`;
 ...
 */
export const MINI_BAR_V4_CSS = `
[data-xen-v4-minibar] { background-color: var(--xen-chart-grid); }
${Array.from({ length: CHART_SERIES_COUNT }, (_, i) => slotRule(i)).join('\n')}
${(['success', 'warn', 'danger'] as ChartToneV4[]).map(toneRule).join('\n')}
`;
 ...
 * assert against.
 */
export const MiniBarV4 = React.forwardRef<HTMLDivElement, MiniBarV4Props>(function MiniBarV4(
  {
    value,
    max = 100,
    slot = 0,
  ...
  injectStyleOnce(MINI_BAR_V4_STYLE_ID, MINI_BAR_V4_CSS);
  ...
  if (!Number.isInteger(slot) || slot < 0 || slot >= CHART_SERIES_COUNT) {
    throw new RangeError(
      `@xenition/ui charts: series ${slot} is outside the ${CHART_SERIES_COUNT}-slot palette. ` +
  ...
  if (loading) {
    return (
      <div ref={ref} className={cn('w-full', className)} style={style} {...rest}>
        <SkeletonV4 variant="rect" width="100%" height={height ?? 'var(--xen-space-sm)'} />
      </div>
    );
  }
  ...
  return (
    <div
      ...
      data-xen-v4-minibar=""
      role="img"
      aria-label={ariaLabel ?? `${formatValue(value)} of ${formatValue(ceiling)}`}
      className={cn(
        'w-full overflow-hidden rounded-[var(--xen-radius-full)]',
        height === undefined ? 'h-sm' : undefined,
        className
      )}
      style={{
    ...
    >
      <div
        data-xen-v4-minibar-fill=""
        // The line's shared "my length is the value" marker: the width eases
        // when the number changes rather than jumping.
        data-xen-v4-chart-fill=""
        data-slot={tone === undefined ? slot : undefined}
        data-tone={tone}
        className="h-full rounded-[var(--xen-radius-full)]"
