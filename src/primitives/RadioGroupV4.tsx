...
export function RadioGroupV4({
  ...
  injectStyleOnce('xen-v4-radio-styles', RADIO_V4_CSS);
  ...
  return (
    <div
      ...
      className={cn(
        ...
        orientation === 'vertical' ? 'flex-col gap-xs' : 'flex-row flex-wrap gap-md',
        className
      )}
    >
      {options.map((o) => (
        <label
          key={o.value}
          className={cn(
            'inline-flex min-h-[var(--xen-space-2xl)] cursor-pointer items-center gap-sm',
            'text-base text-on-surface',
            o.disabled && 'pointer-events-none opacity-[0.38]'
          )}
        >
          <input
            type="radio"
            ...
            className={cn(
              ...
              'rounded-[var(--xen-radius-full)] border border-border bg-surface'
            )}
          />
          {o.label}
        </label>
