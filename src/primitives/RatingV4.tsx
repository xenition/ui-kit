 ...
 * fraction.
 */
export const RatingV4 = React.forwardRef<HTMLSpanElement, RatingProps>(function RatingV4(
  ...
  const pct =
    total > 0 ? Math.round(Math.max(0, Math.min(1, value / total)) * 10000) / 100 : 0;
  ...
  return (
    <span
      ...
      {...rest}
    >
      <span aria-hidden="true" className="relative inline-flex leading-none tracking-[0.1em]">
        <span className="inline-flex text-muted-text">{stars}</span>
        <span
          data-xen-v4-rating-fill=""
          // `accent-text`, not `accent` — a filled star IS the text here.
          className="absolute inset-y-0 left-0 inline-flex overflow-hidden text-accent-text"
          style={{ width: `${pct}%` }}
        >
