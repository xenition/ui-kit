...
export function SegmentedV4({
  ...
  const indicator = useMovingIndicator<HTMLButtonElement>(value, options.length);
  ...
  return (
    <div
      ...
      className={cn(
        'relative inline-flex rounded-[var(--xen-radius-full)] p-xs',
        className
      )}
    >
      {indicator.style !== null && (
        <span
          ...
          aria-hidden="true"
          className="absolute inset-y-xs left-0 rounded-[var(--xen-radius-full)]"
      ...
      {options.map((option) => {
        ...
        return (
          <button
            key={option.value}
            ref={indicator.itemRef(option.value)}
            type="button"
            role="tab"
            ...
            className={cn(
              'relative inline-flex items-center justify-center whitespace-nowrap px-lg',
              'rounded-[var(--xen-radius-full)] font-body text-sm focus-visible:outline-none',
              MIN_TAP_CLASS,
              // `on-surface` on the thumb, `muted` on the rail — the same pair
              // the thumb's own fill is guaranteed against.
              active ? 'font-semibold text-on-surface' : 'font-medium text-muted-text'
            )}
          >
            {option.label}
          </button>
