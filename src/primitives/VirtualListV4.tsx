...
export function VirtualListV4<T>({
  ...
  if (loading) {
    return (
      <div className={cn('flex items-center justify-center p-xl', className)} {...rest}>
        <span
          role="status"
          aria-label="Loading"
          className={cn(
            'inline-block h-5 w-5 animate-spin rounded-[var(--xen-radius-full)]',
    ...
    return (
      <div
        className={cn(
          ...
          'bg-surface p-xl text-center font-body text-sm text-muted-text',
          className
        )}
        {...rest}
      >
