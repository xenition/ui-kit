...
export interface StepListProps {
   ...
   */
  current?: number;
  /** Fires with the index when a step is clicked. Rows are inert without it. */
  ...
  return (
    <ol className={cn('flex flex-col', className)}>
      {steps.map((step, i) => {
        ...
        const body = (
          <div className={cn('flex gap-md', last ? 'pb-0' : 'pb-lg')}>
            <div className="flex flex-col items-center">
              <span
                className={cn(
                  'flex h-8 w-8 shrink-0 items-center justify-center rounded-full',
                  done ? 'bg-primary' : active ? 'border-2 border-primary' : 'border-2 border-border'
                )}
              >
              ...
              */}
              {connector && !last ? <span className="mt-xs w-px flex-1 bg-border" /> : null}
            </div>
