...
import { cn } from './cn';
import { isNumericText } from './internal/v4-data';
...
export function DescriptionsV4({
  ...
  return (
    <dl
      className={cn(
        'grid gap-x-[var(--xen-space-xl)] gap-y-[var(--xen-space-lg)]',
    ...
    >
      {items.map((it, i) => (
        <div key={i} className="flex min-w-0 flex-col gap-0.5">
          <dt className="text-xs font-medium text-muted-text">{it.label}</dt>
          <dd
            className={cn(
              'text-base font-semibold text-on-surface',
              typeof it.value === 'string' && isNumericText(it.value)
