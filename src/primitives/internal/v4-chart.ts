 ...
 * card it sits in.
 */
export const CHART_HUE_OFFSETS: readonly number[] = [0, 250, 90, 320, 200];
 ...
 * theme.
 */
export const CHART_LIGHTNESS: Readonly<Record<ChartScheme, readonly number[]>> = {
 ...
 */
export const CHART_CHROMA: Readonly<Record<ChartScheme, number>> = {
  light: 0.13,
  dark: 0.15,
};
 ...
 * gains chroma.
 */
export const CHART_L_BAND: Readonly<Record<ChartScheme, readonly [number, number]>> = {
  light: [0.44, 0.76],
  dark: [0.49, 0.66],
};
 ...
 */
export const CHART_SCATTER_SERIES_CAP = 3;
 ...
 */
export function maxChroma(L: number, hueDeg: number): number {
 ...
 */
export function chartSlot(nominalL: number, hueDeg: number, targetC: number, scheme: ChartScheme): string {
  const [lo, hi] = CHART_L_BAND[scheme];
  if (maxChroma(nominalL, hueDeg) >= targetC) return oklchToHex(nominalL, targetC, hueDeg);
  let bestL = nominalL;
  ...
  for (let d = 0.02; d <= 0.3; d += 0.02) {
    for (const L of [nominalL + d, nominalL - d]) {
      ...
      if (c >= targetC) return oklchToHex(L, targetC, hueDeg);
      if (c > bestC) {
        bestC = c;
        bestL = L;
      }
 ...
 */
export function chartSeries(brandHex: string, scheme: ChartScheme): string[] {
  const base = hueOf(brandHex);
  const Ls = CHART_LIGHTNESS[scheme];
  const c = CHART_CHROMA[scheme];
  return CHART_HUE_OFFSETS.map((off, i) => chartSlot(Ls[i] as number, (base + off) % 360, c, scheme));
}
 ...
 */
export function chartSequential(brandHex: string, t: number, scheme: ChartScheme): string {
  ...
  const c = Math.min(CHART_CHROMA[scheme], maxChroma(L, hue));
 ...
 * fight a red brand.
 */
export function chartDiverging(brandHex: string, t: number, scheme: ChartScheme): string {
  const hue = hueOf(brandHex);
  ...
  const c = Math.min(CHART_CHROMA[scheme] * mag, maxChroma(L, arm));
 ...
 */
export const CHART_DIRECT_LABEL_MAX = 4;

/* ------------------------------------------------------------------------ *
 * The vocabulary every chart shares
 *
 * These landed after the four build groups did, and each one is here because
 * it was independently redeclared in three or four places while the module was
 * being built in parallel. That is the exact drift `v4-state.ts` exists to
 * prevent — `CHART_AREA_FILL_ALPHA` had already forked to `0.15` on one twin
 * and `0.2` on the other in the BASE line, which is how the fork gets noticed
 * in the first place.
 * ------------------------------------------------------------------------ */
 ...
 */
export const CHART_RAMP_STEPS = 9;
 ...
 */
export const CHART_AREA_FILL_ALPHA = 0.18;

/** What the folded tail of a categorical series list is called. */
export const CHART_OVERFLOW_LABEL = 'Other';
 ...
 */
export function foldChartSeries<T>(items: readonly T[], limit = CHART_SERIES_COUNT): ChartFold<T> {
  if (items.length <= limit) return { kept: [...items], folded: [], didFold: false };
  return {
