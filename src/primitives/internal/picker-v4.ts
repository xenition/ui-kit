...
import { EASE_ENTER, transitionCss, V4_MOTION } from './v4-motion';
import { FIELD_HALO_PERCENT } from './field-v4';
 ...
 * is meant to remove.
 */
export const TAP_TARGET = 'var(--xen-space-2xl)';

/** Tailwind arbitrary values for the tap-target floor, as a class fragment. */
export const TAP_TARGET_CLASS = 'min-h-[var(--xen-space-2xl)] min-w-[var(--xen-space-2xl)]';
 ...
 */
export const RANGE_FILL = 'color-mix(in srgb, var(--xen-primary) 16%, var(--xen-surface))';
 ...
 * `[data-xen-v4-wash]`.
 */
export const BRAND_WASH = RANGE_FILL;
 ...
 */
export const HOVER_FILL = 'color-mix(in srgb, var(--xen-on-surface) 7%, var(--xen-surface))';
...
[data-xen-v4-field] {
  ...
  transition: ${transitionCss(['border-color', 'box-shadow'], PICKER_MOTION.feedback)};
}
...
[data-xen-v4-hover]:hover:not(:disabled) { background-color: ${HOVER_FILL}; }
...
*/
[data-xen-v4-active="true"] { background-color: ${HOVER_FILL}; }
...
/*
  ...
  the glyph: full ${TAP_TARGET} in both axes, out of flow, costing nothing. It
...
*/
[data-xen-v4-band] { background-color: ${RANGE_FILL}; }

/* A brand-touched ground — a live drop target, an icon well. */
[data-xen-v4-wash] { background-color: ${BRAND_WASH}; }
...
[data-xen-v4-slider]:disabled { cursor: default; opacity: ${V4_STATE.disabledContent}; }
...
[data-xen-v4-slider]:focus-visible::-webkit-slider-thumb {
  box-shadow: var(--xen-elevation-card),
    0 0 0 var(--xen-space-xs) color-mix(in srgb, var(--xen-ring) ${FIELD_HALO_PERCENT}%, transparent);
}
