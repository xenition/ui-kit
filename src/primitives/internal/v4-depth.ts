...
import { contrastRatio, ensureContrast, hexToRgb, rgbToHex } from '../../theme/color';
...
export function gradientInk(
  ...
  const worst = (candidate: string): number =>
    Math.min(contrastRatio(candidate, brand.from), contrastRatio(candidate, brand.to));
  ...
  return {
    from: ensureContrast(brand.from, ink, min),
    to: ensureContrast(brand.to, ink, min),
    ink,
  };
}
 ...
 */
export function mixToken(a: string, b: string, t: number): string {
  const x = hexToRgb(a);
  const y = hexToRgb(b);
 ...
 */
export function gradientCss(angle: number, from: string, to: string): string {
 ...
 */
export function shadowCss(token: ElevationToken, k = 1): string {
 ...
 */
export function useOptionalCompiledTheme(): CompiledTheme | null {
  try {
    return useXenitionCompiledTheme();
  } catch {
    return null;
  }
}
 ...
 */
export const MONOGRAM_STEPS = [100, 200, 300, 400] as const;
 ...
 */
export function monogramStep(name: string | undefined): RampStep {
  ...
  const index = Math.abs(hash) % MONOGRAM_STEPS.length;
  return MONOGRAM_STEPS[index] as RampStep;
}
 ...
 */
export function mirrorStep(step: RampStep): RampStep {
  const index = RAMP_STEPS.indexOf(step);
  return RAMP_STEPS[RAMP_STEPS.length - 1 - index] as RampStep;
}
