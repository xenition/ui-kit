...
import { EASE_ENTER, EASE_EXIT, transitionCss, V4_MOTION } from './v4-motion';
 ...
 */
export const MIN_TAP = 'calc(var(--xen-space-2xl) - var(--xen-space-xs))';

/** {@link MIN_TAP} as a Tailwind arbitrary value (spaces are underscores). */
export const MIN_TAP_CLASS = 'min-h-[calc(var(--xen-space-2xl)_-_var(--xen-space-xs))]';

/** {@link MIN_TAP} applied to both axes — for a square target like a page number. */
export const MIN_TAP_SQUARE_CLASS =
  'min-h-[calc(var(--xen-space-2xl)_-_var(--xen-space-xs))] min-w-[calc(var(--xen-space-2xl)_-_var(--xen-space-xs))]';
 ...
 * rhythm.
 */
export const PANEL_MIN_WIDTH_CLASS = 'min-w-[calc(var(--xen-space-2xl)_*_4)]';
...
[data-xen-v4-nav-indicator] {
  transition: transform ${NAV_MOTION.indicator}ms ${EASE_ENTER}, width ${NAV_MOTION.indicator}ms ${EASE_ENTER};
  pointer-events: none;
}

/* A quiet ground for the item under the pointer — feedback, not a highlight. */
[data-xen-v4-nav-item] {
  transition: ${transitionCss(['color', 'background-color', 'border-color'], NAV_MOTION.reveal)};
}
...
[data-xen-v4-nav-panel] {
  background-color: var(--xen-surface);
  box-shadow: var(--xen-elevation-sheet);
  animation: xen-v4-nav-in ${NAV_MOTION.reveal}ms ${EASE_ENTER};
}
/*
  A tooltip bubble.
...
[data-xen-v4-nav-tip] {
  ...
  box-shadow: var(--xen-elevation-card);
  animation: xen-v4-nav-fade ${NAV_MOTION.reveal}ms ${EASE_EXIT};
}
...
@media (prefers-reduced-motion: reduce) {
  ...
  [data-xen-v4-nav-panel] { animation: xen-v4-nav-fade ${NAV_MOTION.reveal}ms ${EASE_EXIT}; }
}
`;
 ...
 * hook succeeding.
 */
export function useMovingIndicator<T extends HTMLElement>(
  activeKey: string,
  itemCount: number
): MovingIndicator<T> {
  const items = React.useRef(new Map<string, T>()).current;
