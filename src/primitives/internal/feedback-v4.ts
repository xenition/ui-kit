 ...
 */
export const TONE_SLOTS: Record<FeedbackTone, ToneSlots> = {
 ...
 */
export const BUSY_MOTION = { spin: 900, pulse: 700 } as const;
 ...
 */
export function toneVar(slot: keyof SemanticColors): string {
 ...
 */
export function mixCss(
  slot: keyof SemanticColors,
  into: keyof SemanticColors,
  amount: number
): string {
...
export function tintCss(slot: keyof SemanticColors, amount: number = TINT): string {
  return mixCss(slot, 'surface', amount);
}

/** {@link mixCss} as a Tailwind arbitrary value (spaces become underscores). */
export function mixArbitrary(
  slot: keyof SemanticColors,
  into: keyof SemanticColors,
  amount: number
): string {
...
export function tintArbitrary(slot: keyof SemanticColors, amount: number = TINT): string {
