...
import { ZEBRA_MIX, zebraCss } from './v4-data';
import { V4_STATE, stateCss } from './v4-state';

/** The `<style>` id the V4 chrome components share. Injection is idempotent. */
export const CHROME_V4_STYLE_ID = 'xen-v4-chrome-styles';

/** An M3 easing quadruple as a CSS `cubic-bezier()`. */
export function easingCss(easing: readonly number[]): string {
  return `cubic-bezier(${easing.join(', ')})`;
}

/** `standard` — a state change: a row tinting, a fill arriving. */
export const EASE_STANDARD = easingCss(MOTION.easingStandard);
/** `enter` — something arriving on screen. */
export const EASE_ENTER = easingCss(MOTION.easingEnter);
 ...
 */
export const CHROME_V4_CSS = `
[data-xen-v4-chrome] {
  transition: background-color ${MOTION.quick}ms ${EASE_STANDARD}, color ${MOTION.quick}ms ${EASE_STANDARD};
}
...
[data-xen-v4-chrome="on-surface"]:active:not(:disabled):not([aria-disabled="true"]) {
  background-color: ${stateCss('var(--xen-on-surface)', 'var(--xen-surface)', 'pressed')};
}
...
[data-xen-v4-chrome="primary"]:active:not(:disabled):not([aria-disabled="true"]) {
  background-color: ${stateCss('var(--xen-primary)', 'var(--xen-surface)', 'pressed')};
}
...
[data-xen-v4-chrome="danger"]:active:not(:disabled):not([aria-disabled="true"]) {
  background-color: ${stateCss('var(--xen-danger)', 'var(--xen-surface)', 'pressed')};
}
/*
...
[data-xen-v4-chrome="filled-primary"]:hover:not(:disabled):not([aria-disabled="true"]) {
  background-color: ${stateCss('var(--xen-on-primary)', 'var(--xen-primary)', 'hover')};
}
[data-xen-v4-chrome="filled-primary"]:active:not(:disabled):not([aria-disabled="true"]) {
  background-color: ${stateCss('var(--xen-on-primary)', 'var(--xen-primary)', 'pressed')};
}
[data-xen-v4-chrome="filled-danger"]:hover:not(:disabled):not([aria-disabled="true"]) {
  background-color: ${stateCss('var(--xen-on-danger)', 'var(--xen-danger)', 'hover')};
}
[data-xen-v4-chrome="filled-danger"]:active:not(:disabled):not([aria-disabled="true"]) {
  background-color: ${stateCss('var(--xen-on-danger)', 'var(--xen-danger)', 'pressed')};
}
