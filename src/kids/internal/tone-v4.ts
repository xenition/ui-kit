 ...
 */
export const KIDS_CARD_CLASS = 'rounded-[var(--xen-radius-lg)] p-lg';
 ...
 */
export const KIDS_CARD_GROUND_CLASS = 'border border-border bg-card text-on-card';
...
export const DISABLED_CLASS = 'pointer-events-none opacity-[0.38]';
 ...
 */
export function cardStateVars(): React.CSSProperties {
  return stateGroundVars('var(--xen-card)', 'var(--xen-on-card)') as React.CSSProperties;
}

/** The same, for a control that sits directly on the page rather than a card. */
export function surfaceStateVars(): React.CSSProperties {
  return stateGroundVars('var(--xen-surface)', 'var(--xen-on-surface)') as React.CSSProperties;
}
 ...
 */
export function meterAria(parts: MeterParts, valueText: string): MeterAria {
 ...
 * currency is owed.
 */
export function allowanceMoney(amount: number, currency = '$', locale?: string): string {
  if (!Number.isFinite(amount)) return '—';
  if (ISO_CURRENCY.test(currency)) return formatMoney(Math.round(amount * 100), currency, locale);
