...
export function hexToRgb(hex: string): RGB {
  if (!isValidHex(hex)) {
    throw new Error(
      `Invalid hex color ${JSON.stringify(hex)} — expected "#rgb" or "#rrggbb" (e.g. "#7C3AED").`
    );
  }
