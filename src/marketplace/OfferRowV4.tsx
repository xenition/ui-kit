 ...
 */
export const OfferRowV4 = React.forwardRef<HTMLDivElement, OfferRowV4Props>(function OfferRowV4(
  {
    party,
    amountCents,
    currency = 'USD',
  ...
  return (
    <div
      ref={ref}
      data-xen-offer-row=""
      aria-label={`${party}, ${look.label}, ${amount}`}
      className={cn('flex w-full flex-col', className)}
      {...rest}
    >
      <div data-xen-v4-row="" className={cn(ROW_V4_BASE_CLASS, rowHeightClass(supporting))}>
        {showAvatar ? (
        ...
        <span className={ROW_V4_TEXT_CLASS}>
          <span className="flex min-w-0 items-center gap-sm">
            <TextV4 size="base" weight="semibold" tone="onSurface" numberOfLines={1} className="min-w-0">
              {party}
            </TextV4>
      ...
      {showActions ? (
        <div className="flex gap-sm px-md pb-sm">
          {onAccept != null ? (
            <ButtonV4 variant="primary" tone="success" size="sm" onClick={onAccept} className="flex-1">
              Accept
            </ButtonV4>
          ) : null}
          {onCounter != null ? (
            <ButtonV4 variant="outline" size="sm" onClick={onCounter} className="flex-1">
              Counter
            </ButtonV4>
          ) : null}
          {onDecline != null ? (
            <ButtonV4 variant="ghost" tone="danger" size="sm" onClick={onDecline} className="flex-1">
              Decline
            </ButtonV4>
          ) : null}
        </div>
